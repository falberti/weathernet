...
# Obtained from @BotFather, not generated by this project -- see
# subscriptions/README or the top-level README for the exact steps.
# Empty means the telegram-bot service/digest command can't reach the
# Bot API at all; both fail loudly rather than silently doing nothing.
TELEGRAM_BOT_TOKEN = os.environ.get("TELEGRAM_BOT_TOKEN", "")
# The bot's @username (without the @) -- used only to build the
# t.me/<username> link shown on the public page, never passed to the
# Bot API itself.
TELEGRAM_BOT_USERNAME = os.environ.get("TELEGRAM_BOT_USERNAME", "")
# How close a subscription's location must be to a probe (km, great-
# circle) for that probe to count as "close enough" -- see
# subscriptions/matching.py. 15km: temperature/humidity/pressure stay
# reasonably representative at that scale; air quality from a single
# BME680 is much more hyperlocal, but there's no better distance-based
# proxy with single-sensor probes.
SUBSCRIPTION_MAX_DISTANCE_KM = float(os.environ.get("SUBSCRIPTION_MAX_DISTANCE_KM", "15"))
# Required by Nominatim's usage policy for every request -- identifies
# this application, not a per-user value.
